...
- Run `golangci-lint` as an executable (installed via the `Brewfile`) instead of a `go tool`, and adjust `go.mod`/`go.sum` accordingly.
...
- `stave --clean` also sweeps Stave-generated binaries from the legacy `~/.stavefile` directory when it still exists, so binaries compiled by older versions do not linger after cleaning. Unrelated files in cache directories are preserved, and unsafe cache paths are rejected.
...
- Remove `gomoddirectives` exclusion in `.golangci.yml`.
...
- Bump Go version to `1.25.13` (and update dependencies).

## [0.16.12] - 2026-08-13

### Changed

- Bump `yaklabco/direnv` to `v2.37.1.20`.
...
- Moved `goreleaser` back from a Go `tool` directive to a `Brewfile` dependency, to avoid bumping Go version to most recent major version. Downgraded back to Go `1.25.12`.
...
- Added `sh.PrepCmd(With)(V)` family of functions for preparing `exec.Cmd` instances with full stave bells & whistles, but without actually running them. Useful for spawning background tasks without waiting for their completion.
...
- Improve new tests for working-directory functionality added in version [0.15.0](#0150---2026-06-26).
- Bump all updatable Go dependencies to their latest versions as of this date.
...
(Superseded by [0.14.15](#01415---2026-06-08).)

## [0.14.13] - 2026-06-04

### Changed

- Bump Go version to `1.25.11` (and update dependencies).

## [0.14.12] - 2026-05-26

### Changed

- Maintenance release to circumvent corrupted Go proxy entries.

## [0.14.11] - 2026-05-25

### Changed

- Bump `yaklabco/direnv` to `v2.37.1.17`.
- Bump all updatable Go dependencies to their latest versions as of this date.
...
- Port in PRs from the <https://github.com/magefile/mage> project:
  - <https://github.com/magefile/mage/pull/544>: fix bug 537 - backticks in comments break output
  - <https://github.com/magefile/mage/pull/552>: make `-i/--info` no long require compiling

## [0.14.6] - 2026-04-08

### Changed

- Bump Go version to `1.25.9` (and update dependencies).
...
- Bump `yaklabco/direnv` to `v2.37.1.14`.
...
- Bump Go version to `1.24.12`, updated 3rd-party Go dependencies.
...
- Bump `yaklabco/direnv` to `v2.37.1.12`.
...
[unreleased]: https://github.com/yaklabco/stave/compare/v0.16.18...HEAD
...
[0.16.10]: https://github.com/yaklabco/stave/compare/v0.16.9...v0.16.10
[0.16.9]: https://github.com/yaklabco/stave/compare/v0.16.8...v0.16.9
...
[0.9.1]: https://github.com/yaklabco/stave/compare/v0.9.0...v0.9.1
