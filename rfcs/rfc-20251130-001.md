...
- Backwards-compatible support for existing `STAVEFILE_*` and `MAGEFILE_*` environment variables.
...
- A **non-breaking migration path** from existing `STAVEFILE_*` / `MAGEFILE_*` env-var-only configuration.
...
- Removing or deprecating `STAVEFILE_*` / `MAGEFILE_*` environment variables in this iteration.
...
graph TD
    ...
    D[Environment Vars<br/>STAVEFILE_*/MAGEFILE_*] --> F
...
sequenceDiagram
    ...
    CFG->>CFG: Fallback to MAGEFILE_* if unset
...
6. Environment variables are bound (unless `SkipEnv`) with both:
   ...
   - Legacy: `MAGEFILE_*` (used if the preferred one is unset).
...
- **Environment variables**:
  ...
  - Legacy `MAGEFILE_*` env vars continue to work as a fallback if `STAVEFILE_*` is unset.
...
- **Loading behavior**:
  ...
  - `TestLoad_LegacyEnvironmentVariables` – `MAGEFILE_*` fallback.
...
4. **Optional deprecation (long-term)**:
   - If desired, in a later major version we may:
     - Warn when using `MAGEFILE_*` env vars without `STAVEFILE_*`.
