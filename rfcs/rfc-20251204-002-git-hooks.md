...
2. **Git invokes the `.git/hooks/pre-commit` script:** Git determines the effective hooks directory (either `.git/hooks` or the directory configured via `core.hooksPath`) and checks for an executable `pre-commit` file. If present, Git runs this script synchronously; if it exits non-zero, Git aborts the commit and surfaces the hook’s error output to the user.
   Git determines the effective hooks directory (either `.git/hooks` or the directory configured via `core.hooksPath`) and checks for an executable `pre-commit` file. If present, Git runs this script synchronously; if it exits non-zero, Git aborts the commit and surfaces the hook’s error output to the user.
...
6. **The hooks runtime resolves the configured targets for `pre-commit`:** Using the loaded `HooksConfig`, the runtime looks up the entry for the `pre-commit` hook and converts each item into an internal representation (target name, arguments, and flags such as `passStdin`). If no configuration exists for `pre-commit`, the runtime simply returns success so that unconfigured hooks do not unexpectedly block commits.
   Using the loaded `HooksConfig`, the runtime looks up the entry for the `pre-commit` hook and converts each item into an internal representation (target name, arguments, and flags such as `passStdin`). If no configuration exists for `pre-commit`, the runtime simply returns success so that unconfigured hooks do not unexpectedly block commits.

7. **Configured Stave targets (e.g. `fmt`, `lint`) are executed:** For each configured target, the runtime invokes the existing Stave target execution pipeline in order, passing through any additional CLI arguments and stdin behavior defined in the hook config. Targets are run sequentially in this first iteration, and the runtime records the exit code and basic diagnostics for each one.
   For each configured target, the runtime invokes the existing Stave target execution pipeline in order, passing through any additional CLI arguments and stdin behavior defined in the hook config. Targets are run sequentially in this first iteration, and the runtime records the exit code and basic diagnostics for each one.

8. **The hooks runtime returns an aggregated exit code to the `stave hooks` CLI:** After all targets have run—or as soon as one fails, depending on policy—the runtime computes a single exit status, typically the exit code of the first failing target. It may also assemble a concise summary message (e.g. which target failed) so that the CLI can present a clear, actionable error to the user.
   After all targets have run—or as soon as one fails, depending on policy—the runtime computes a single exit status, typically the exit code of the first failing target. It may also assemble a concise summary message (e.g. which target failed) so that the CLI can present a clear, actionable error to the user.
