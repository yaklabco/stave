...
| `--multiline` |       | `false`         | Retain line returns in help text              |
...
| `--clean`     |       | `false`         | Remove cached compiled binaries, including any in the legacy `~/.stavefile` directory |
...
| `STAVEFILE_CACHE`      | `cache_dir` config option |
