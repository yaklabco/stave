...
Projects that support both Husky and native Stave hooks can use the `Hooks` target to switch between them.
...
- Creates `stave.yaml` with default hooks configuration if it doesn't exist
- Unsets `core.hooksPath` (uses `.git/hooks/`)
- Installs Stave-managed hook scripts

**`stave Hooks husky`:**

- Removes any Stave-managed hooks from `.git/hooks/`
- Sets `core.hooksPath` to `.husky`
- Ensures Husky hook scripts are executable

### Adding to Your Project

Add this target to your `stavefile.go`:

```go
// Hooks configures git hooks to use either "husky" or "stave" (native).
func Hooks(system string) error {
