...
func runConfigShow(stdout, stderr io.Writer) int {
	...
	fmt.Fprintln(stdout, "# Effective Stave Configuration")
	if cfg.ConfigFile() != "" {
		fmt.Fprintf(stdout, "# Loaded from: %s\n", cfg.ConfigFile())
	} else {
		fmt.Fprintln(stdout, "# (using defaults, no config file found)")
	}
	fmt.Fprintln(stdout)
	fmt.Fprintf(stdout, "cache_dir: %s\n", cfg.CacheDir)
	fmt.Fprintf(stdout, "go_cmd: %s\n", cfg.GoCmd)
	fmt.Fprintf(stdout, "verbose: %v\n", cfg.Verbose)
	fmt.Fprintf(stdout, "debug: %v\n", cfg.Debug)
	fmt.Fprintf(stdout, "hash_fast: %v\n", cfg.HashFast)
	fmt.Fprintf(stdout, "ignore_default: %v\n", cfg.IgnoreDefault)
	fmt.Fprintf(stdout, "enable_color: %v\n", cfg.EnableColor)
	fmt.Fprintf(stdout, "target_color: %s\n", cfg.TargetColor)
